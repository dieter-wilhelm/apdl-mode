...
** Idosyncracies
  - No function definitions <- write `command' files (suffix: .mac),
    or call a macro (arbitrary suffix) with `*use'
  - No direct array to file export in GUI mode <- write command file
    for *vwrite, or use a (lookup) table for this purpose
  - *vwread does not work with C format specifiers in contrast to *vwrite
  - Still (v14) no round function in sight, someting like nint(max*1e3)/1e3 can do
...
** File zoo/types (operations guide) under Gnu/Linux?
