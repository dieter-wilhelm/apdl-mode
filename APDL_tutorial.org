...
#+end_src
    which gives the comment at the right indentation
#+begin_src ansys :tangle no
  !! Type "M-;" in the empty line below
  *if,I,eq,2,then
    !! 
...
#+begin_src ansys :exports none
  !@@ -- The ANSYS `interactive graphics' frame  --
#+end_src
#+begin_src ansys
  !! Type "C-c C-g" to open the ANSYS `interactive' graphics frame.
  !! Then type e. g. "C-c C-q eplot <RET>".
#+end_src
...
Unfortunately it is not possible to interact directly with the
graphics except with APDL commands and through the Pan/Zoom/Rotate
dialog!
...
#+end_src
*Warning:* As long as the dialog is open *no* communication is
 possible!
