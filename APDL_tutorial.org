...
** Extensible APDL templates
   (*Everything* is extensible in Emacs and ANSYS-Mode. ;-) Some
   templates are collections of (all) available commands to certain
   subjects.  They are more intended to be previewed.  Others are
   self-contained "mini simulations" or skeletons for starting a new
   project. These are more apt for insertion.
#+begin_src ansys :exports none
  !@ --- Extensible APDL templates ---
#+end_src
...
#+end_src
#+begin_src ansys
  !! Insert below the following line the `ansys-skeleton-structural-template'!
  !! Type "C-u C-c C-s" for insertion instead of previewing.
#+end_src
...
#+begin_src ansys
  ...
  !! FILENAME: /home/dieter/ansys-mode/trunk/APDL_tutorial.org[*Org Src APDL_tutorial.org[ ansys ]*]
...
#+end_src
Helful to get an overview of large APDL file.
#+begin_src ansys :tangle no
  !@ ==============================
  !@ --- Header ---
  !@ ==============================...
...
#+end_src
    which places the comment characters already with the right indentation
#+begin_src ansys :tangle no
  !! Type "M-;" in the empty line below the code
  *if,I,eq,2,then
    !! 
...
#+end_src
#+begin_src ansys
  !! If you input "M-;" in a line with an existing comment
  !! then the cursor skips to the comment beginning.
  nsel,s,loc,x,0,1         ! this is an `inline comment'
#+end_src
Is this not nicely thought out from Emacs' developers!
...
#+begin_src ansys
  ...
  !! Emacs indents and places comment characters in the new line
#+end_src
...
#+begin_src ansys
  ...
  P = ARNE
#+end_src ansys
    Note that the cursor is conveniently placed inside the
    parentheses.  The completions are unique in above examples and
    expand thus:
#+begin_src ansys :tangle no
...
#+end_src ansys
*** Dynamic completion
    That means completion on the basis of the window's content, the
    following example will show what this means.
#+begin_src ansys :exports none
  !@@ -- Dynamic completion --
#+end_src
# !!    type *M-/*
...
#+end_src
** Logical blocks (control statements in "lesser" programming languages ;-)
#+begin_src ansys :exports none
...
#+end_src
#+begin_src ansys
   !! Insert in between the following block another `*if' statement!
...
et,Steel,beam3 $ et,Alu,shell91  !deprecated
#+end_src
...
#+end_src
  Finally switch off unnecessay stuff: Emacs' menu-, tool- and scroll-bar :-)
#+begin_src ansys
   !! Erase above APDL code and reinsert the `structural' template below
   !! ("M-< C-w")
#+end_src
...
#+end_src
#+begin_src ansys
   !! Type "C-c C-t"!  And select the required license type
   !! Use the <TAB> key to see the predefined license types
#+end_src
** Display the license status
   highlighted in red is your chosen license type
#+begin_src ansys :exports none
...
#+end_src
#+begin_src ansys
  !! Place the cursor below `solve' command and
...
except with APDL commands and through the Pan/Zoom/Rotate dialog!  And
secondly: As long as the dialog is open *no* other communication is
possible! And thirdly: The size of the graphics is fixed. And
fourthly: You must replot every time the graphics frame was hidden by
...
#+end_src
#+begin_src ansys
  !! Type "C-c C-q" and send your input to the interpreter!
