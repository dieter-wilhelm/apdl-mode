...
** Version 16.1.2, Not yet released
   - Realised sending code to the ANYSClassics GUI as well.

   - Checking fast the available licenses (with "C-c C-l") is now also
     available on the Windows.

   - Reorganised and simplified the installation and configuration.

   - The command `C-c C-j' does now recognise blocks, which is to say
     it copies or sends complete logical blocks to the interpreter
     instead of only this code line.
