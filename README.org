...
** Development and source code installation
   ...
   #+begin_src sh
     git https://github.com/dieter-wilhelm/apdl-mode.git
   #+end_src
   or download the ZIP archive from Github and add the following line
   #+begin_src elisp
   (require 'apdl-mode)
   #+end_src
   to your initialisation file (the load-path must be set as well).
* Usage
