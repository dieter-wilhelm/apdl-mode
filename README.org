...
** Pin-pointing the relevant ANSYS documentation
   Without the detour of searching in the ANSYS Help Viewer or in the
   online help you can directly trigger the relevant manual entry from
   your APDL session.  This works for APDL commands, element names and
   other manual topics.  The help topics are shown in a web browser.
   (To that end you must have installed the local ANSYS documentation
   and configured APDL-Mode.)

   #+caption: Browsing the manual in a web browser (here with EWW in GNU-Emacs).
   [[file:doc/browse_manual.png][file:d:/apdl-mode/doc/browse_manual.png]]
