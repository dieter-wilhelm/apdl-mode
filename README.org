...
# or via command line way, e. g.: "wget
# http://ansys-mode.googlecode.com/files/ansys-mode-16.1.1.tgz".

The some code (completions, help strings etc.) is based on the latest
ANSYS version.  Besides the sources and Emacs you will find also in
the download section two tutorials.  For an immediate access have a
look at the
