...
** Manual installation
   ...
   2) Download the latest APDL-Mode's tar package from the [[https://github.com/dieter-wilhelm/apdl-mode/releases/tag/20.1.0][Github]]
      release page.
   3) Install the package within Emacs: Please type `M-x
...
** Development and source code installation
   Please clone the APDL-Mode git archive
   #+begin_src sh
...
* Configuration and Customisation
  ...
  installation dependent configurations (for regular Ansys
  installations.  It uses by default the highest installed Ansys
  version.)
...
* Bugs
  - Write an email to the mode maintainer, please trigger a bug report
    form from the APDL menu or by calling the function
    `apdl-submit-bug-report'.  Even if you are not able to send emails
    directly via Emacs, this is a template with valuable information
    like mode settings and Emacs internals.

  - You might also issue a bug report at APDL-Mode's [[https://github.com/dieter-wilhelm/apdl-mode/issues][issue site]].

  - Or you can leave comments and hints at the APDL-Mode page of
    [[https://www.emacswiki.org/emacs/APDLMode][Emacs Wiki]].
