...
** Short instructions
*** For Windows systems without Emacs (or older dated versions)

Download the latest archive with binaries of Emacs-24.5 (bundled with
a pre-configured ANSYS-Mode) at [[https://github.com/dieter-wilhelm/ansys-mode/releases][GitHub]] and unpack it anywhere on your
system (you do not need administrator rights).  For the next steps
please read the included introductory tutorial.
...
above GitHub releases page and unpack it anywhere on your system.

**** Direct way; no file configuration necessary
Start Emacs, then load `A-M.el' by typing "M-x" (first) the <Alt> key
