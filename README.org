...
Download the latest archive with a pre-compiled Emacs-24.5 (bundled
with a pre-configured ANSYS-Mode) at above GitHub site and unpack it
anywhere on your system (you do not need administrator rights).  For
the first steps please read the introductory tutorial.

*** For a GNU-Linux system without Emacs

Please download the source archive
~ansys-mode-16.1.2.emacs-24.5.tar.xz~ from
...
above GitHub site and unpack it anywhere on your system.
