...
** Bugs
   ...
   - Do we need to set environment license variables?  When
     environment var isn't set in .profile, .bash_login nor in
     .bash_profile, for -start-ansys, -launcher it is
     necessary. Especially if one wants to overwrite the file?!...
   - C-c C-j tries to send a next line in the LAST line
...
** Freeze
   - list of all new functions in -bug-report
** Wish-list
   - We need `'' for strings and character parameters,`%' for
     parameter substitution,`(' for functions, % not yet working with
     electric-pair-mode
   ...
   - MinGWin: FindWindow +  ActivateWin + SendKeys?
