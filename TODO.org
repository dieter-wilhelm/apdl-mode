...
* TODO Version 20.3
** Todos, bugs
   - Check the LICENSES file links!
   - Check templates (formatting changed strongly!)
   - Update and centralise DOCUs
   - C-u C-c C-b in browser for "All"stuff doesn't skip to subjects..,
     does it for local help?
   - M-C-i is not consistent with M-?! The mix of Ansys v195 and docu
     v201 version..
** minor bugs
   - what is doc/default.org? configuration for /usr/share/emacs/site-lisp.
     => remove from package and put it in .gitignore
...
*** misc
    1) Melpa integration <= 2021?
       -wb-template.el checkdoc, p-lint: whitespace-mode OK
       -config.el
       -keyword.el
       -initialise.el
       -mode-pkg.el
...
** checks
   ...
   - changed file size without warning to 30 MB what is Emacs-26
     using?
** Release proceedures
   Attention: in apdl_keywords.txt there are many whitespace
   ...
   checkdoc, flycheck etc. : from makefile
    ...
    # for _mode_version
    (tags-query-replace "20\.2\.0" "20.2.1")
    # copyright year
    (tags-query-replace "- 2020" "- 2021")
    # ansys version
    (tags-query-replace "v195" "v201")
    # emacs
    (tags-query-replace "26\.3" "27.1")
    (tags-search "((emacs \"25.1\"))")
    # (tags-query-replace "((emacs \"25\"))" "((emacs \"25.1\"))")
