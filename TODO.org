...
** Todos, bugs, checks
   - C-u C-c C-b in browser for "All"stuff doesn't skip to subjects..,
     does it for local help? NO it doesn't
   ...
   - Check templates (formatting changed strongly!) especially
     -compilation-template!  Indent is not correct., -skeleton-function!?
   - Update and centralise DOCUs, info style?
     - do we need a short help, yes for installation + config + first steps
     - shift org-files to doc, except README.org: Focus: Intro +
...
*** templates
    ...
    - do we need inter-region templates?
*** misc
    1) open EWW in another window!
    2) why initialising anew any time -mode is called? especially with
       C-c C-s!
    3) Travis CI continuous integration!
    4) https://github.com/marketplace/coveralls
    5) find nice style css stuff for html export?
    ...
    8) what is .gitattributes
    9) Melpa readme
    10) merge config branch and stashes
** Done
