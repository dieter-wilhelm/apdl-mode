...
** Wish-list
   - Change versioning scheme to 161.2
   ...
   - Remove M-( when Emacs > 24, document this in usage, substitute
     with electric-pair-mode? We need `'' for strings and character
     parameters,`%' for parameter substitution,`(' for functions
