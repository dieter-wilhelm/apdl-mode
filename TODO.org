...
** Wish-list
   ...
   - saving option for -license-file, -lmutil, -program, ...
   - Check for valid license types for the solver
   - start WB
   - start anslic_admin, with launcher redundant
   - start launcher
   - license and template status line always in the first visible line
     of buffer (in-place-annotations?)
   - -license-status interactive, specifying a temporal license type,
     optional filters for output?
   - Licensing initialisation, read \ANSYS Inc\Shared
     Files\Licensing\ansyslmd.ini
   - Documentation of installation and configuration!
   - C-c C-c checking blocks
   - MinGWin: FindWindow +  ActivateWin + SendKeys?
   - include ../ansys/apdl?
   - ../apdl/start161.ans
   - make version numbers string into integers
