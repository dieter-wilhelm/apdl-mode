...
* 20.1.0
  ...
  - open EWW in another buffer
  - open apdl manual in a EWW buffer

   What is apdl-browse-apdl-guide? => the APDL manual
   a-b-a-g => a-b-ANSYS-APDL-manual

** bugs
   - The ANSYS menu C-c C-l is not greyed out, when apdl-license-file
     is not found
...
** wishes
   - are the material properties documented, matlib?  Matlib is used
     in -template.el, document directory somewhere
