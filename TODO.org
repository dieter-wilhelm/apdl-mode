...
* Deficiencies:
  ...
  - Highlighting :: An apostrophy clashes with the ANSYS "char"
                    ...
                    the menu bar
  - Highlighting :: help overlay is only removed after 2 min if the
                    buffer is active at that time!
...
* 16.1.2:
** Bugs
   - -reload-ansys-mode is only working in the directory where
     -mode.el is sitting
   - Correct the creep data with correct young's modulus and total
   ...
   - Error running timer: (wrong-type-argument overlayp nil): when not in ANSYS buffer!
   - *mwrite *cfclos M-? shows help for *mwrite, when cursor behind
     *cfclos and in the last line without \n
** Wish-list
   - Targeting AnsysClassics
   - Targeting ANSYS Aim
   - Add WorkBench (Aim) concepts to APDL translation
   ...
   - Variables are not highlighted in -skeletons?, *create file
     tmp,mac: macro call: tmp, is not highlighted
   ...
   - -show-command-parameters indicating at which parameter position
     ...
     - check this for commented-out commands
