...
# CAD Kernel of spaceclaim is ACIS (licensed from Dassault)

#+STARTUP: showall
* 20.2.0 Release
  - start the APDL manual also online
  - mark is active :> deactivate mark!
  - first time start: file-mode specification error:
    (wrong-type-argument)? the same with templates!!

  - WB template system => template branch
    - dispatcher / selctor : keyword search
...
** Todos
   - apdl function sets region!
   - 1/en, en is variable but fraction not fontified?
   - what is the advantage of ARG1-9? => parameter system
   - C-c C-s in *APDL code* file-name-extension: Wrong type argument:
     stringp, nilError during redisplay: (jit-lock-function 1)
...
** checks
   ...
   - Check C-c C-b, seems not to work for "v201"
     apdl-current-ansys-version
** wishes
   - open EWW in another window!
   - ANSYS uses sinve version 19. by default its online help.  Trigger
     the ANSYS online manual, if there's no local docu for C-c
     C-b. You need to log into the ANSYS portal to read the online
     tutorial!  This address should work if you already logged into
     the help
     https://ansyshelp.ansys.com/account/secured?returnurl=/Views/Secured/corp/v195/ans_cmd/Hlp_C_NSEL.html
     - Ansys help configuration:
   - Melpa integration
