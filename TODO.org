...
* TODO Version 20.X
** minor bugs
   - Remove times stamp in *License* buffer when updating with `g'.
...
*** templates
    ...
    - do we need interregion templates?
    - what is the advantage of ARG1-9? => parameter system, should I
      include ARG[1-9] optionally?
*** misc
    1) Melpa integration <= 2021?
    2) open EWW in another window!
    3) why initialising anew any time -mode is called?
    4) find nice style css stuff for html export?
    5) integrate new Ansys logo in A-M logo and reverted back from ANSYS
       to Ansys.
    6) docu in info mode
    7) how about deleting the unused branches?
    8) Start optionally runbWB2 --aim, Discovery AIM
    9) package-lint
       - problems with prefix names and default key bindings...
    10) ANSYS is often used synonymous for ANSYS MAPDL solver /
	interpreter or ANSYS Classics GUI
    11) checkdoc, flycheck etc. : from makefile
** Proceedures
    # ansys
    (tags-query-replace "v193" "v201")
    # emacs
    (tags-query-replace "26\.3" "27.1")
    # for _mode_version
    (tags-query-replace "20\.1\.0" "20.2.0")
    (tags-query-replace "ansys+emacs2020.png" "ansys+emacs2020-03.png")
#    (tags-query-replace "ANSYS" "Ansys") done
    # copyright
    (tags-query-replace "- 2020" "- 2021")

* TODO 20.2.0 Release
  DEADLINE: <2020-04-08 Mi>
  Feature freeze [2020-02-27 Do]: Testing of the master branch
  Update documentation, html brutally outdated, ..
  - gh-pages:
    - create a README.org copy from index.org?: Done
    - makefile for htmls or is it org-mode publish: It is.
** Todos, bugs
** checks
   - compile
   - check C-u C-c C-b with various topics )=> Works partly ;-)
