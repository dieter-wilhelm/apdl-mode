...
;; Time-stamp: "2006-12-01 21:58:42 dieter"
...
font-lock-function-name-face    keep)   ("\\(STRSUB\\)\\s-*("   1
font-lock-function-name-face    keep)   ("\\(STRCAT\\)\\s-*("   1
font-lock-function-name-face   keep)   ("\\(STRFILL\\)\\s-*("   1
font-lock-function-name-face   keep)   ("\\(STRCOMP\\)\\s-*("   1
font-lock-function-name-face   keep)   ("\\(STRLEFT\\)\\s-*("   1
...
font-lock-variable-name-face   keep)   ("\\<\\(SHELL51\\)\\>"   1
...
font-lock-variable-name-face   keep)   ("\\<\\(SOLID64\\)\\>"   1
...
font-lock-variable-name-face   keep)   ("\\<\\(SHELL143\\)\\>"  1
...
*CFWRITE, Command" "*CREATE - Opens (creates) a macro file.
...
*DIM, Par, Type, IMAX, JMAX, KMAX, Var1, Var2, Var3, CSYSID" "*DO - Defines the beginning of a do-loop.
*DO, Par, IVAL, FVAL, INC" "*DOWHILE - Loops repeatedly through the next *ENDDO command.
*DOWHILE, Par" "*ELSE
*ELSE - Separates the final if-then-else block." "*ELSEIF - Separates an intermediate if-then-else block.
*ELSEIF, VAL1, Oper, VAL2" "*END
*END - Closes a macro file." "*ENDDO
*ENDDO - Ends a do-loop and starts the looping action." "*ENDIF
*ENDIF - Ends an if-then-else." "*EVAL - Evaluates hyperelastic constants.
*EVAL, ETYPE, EVPARM, CONST, XMIN, XMAX, XVAL, ECALC" "*EXIT
*EXIT - Exits a do-loop." "*GET - Retrieves a value and stores it as a scalar parameter or part of an array parameter.
...
*LIST, Fname, Ext, --" "*MFOURI - Calculates the coefficients for, or evaluates, a Fourier series.
*MFOURI, Oper, COEFF, MODE, ISYM, THETA, CURVE" "*MFUN - Copies or transposes an array parameter matrix.
*MFUN, ParR, Func, Par1" "*MOONEY - Calculates Mooney-Rivlin hyperelastic constants from test data.
*MOONEY, STRAIN, STRESS, --, CONST, CALC, SORTSN, SORTSS, Fname, Ext" "*MOPER - Performs matrix operations on array parameter matrices.
*MOPER, ParR, Par1, Oper, Par2, Par3, kDim, --, kOut" "*MSG - Writes an output message via the ANSYS message subroutine.
*MSG, Lab, VAL1, VAL2, VAL3, VAL4, VAL5, VAL6, VAL7, VAL8" "*MWRITE - Writes a matrix to a file in a formatted sequence.
*MWRITE, ParR, Fname, Ext, --, Label, n1, n2, n3" "*REPEAT - Repeats the previous command.
...
*SET, Par, VALUE, VAL2, VAL3, VAL4, VAL5, VAL6, VAL7, VAL8, VAL9, VAL10" "*SREAD - Reads a file into a string array parameter.
...
*VCUM, KEY" "*VEDIT - Allows numerical array parameters to be graphically edited.
...
/INPUT, Fname, Ext, --, LINE, LOG" "/INQUIRE - Returns system information to a parameter.
...
/LSYMBOL, X, Y, SYMANG, SYMTYP, SYMSIZ, KEYBMP" "/MAIL - Mails file to the specifed address.
/MAIL, --, Address, Fname, Ext" "/MENU - Activates the Graphical User Interface (GUI).
/MENU, Key" "/MPLIB - Sets the default material library read and write paths.
...
/PMACRO - Specifies that macro contents be written to the session log file." "/PMETH - Activates the p-method solution options in the Graphical User Interface (GUI).
...
/RGB, Kywrd, PRED, PGRN, PBLU, N1, N2, NINC, NCNTR" "/RUNST
...
/STITLE, NLINE, Title" "/SX
/SX - Enters the Variational Technology preprocessor." "/SYP - Passes a command string and arguments to the operating system.
...
ANFLOW, NFRAM, DELAY, NCYCL, TIME, SPACING, SIZE, LENGTH" "ANHARM - Produces a time-transient animated sequence of time-harmonic results (ANTYPE,HARMIC).
...
ANSOL, NVAR, NODE, Item, Comp, Name, Mat, Real, Ename" "ANTIME - Produces a sequential contour animation over a range of time.
...
BUCOPT, Method, NMODE, SHIFT, LDMULTE" "C*** - Places a comment in the output.
C***, Comment" "CALC
CALC - Specifies \"Calculation settings\" as the subsequent status topic." "CBDOF - Activates cut boundary interpolation (for submodeling).
...
CELIST, NEQN1, NEQN2, NINC, Nsel" "CENTER - Defines a node at the center of curvature of 2 or 3 nodes.
...
CHKMSH, Comp" "CIRCLE - Generates circular arc lines.
CIRCLE, PCENT, RAD, PAXIS, PZERO, ARC, NSEG" "CLOCAL - Defines a local coordinate system relative to the active coordinate system.
...
CMPLOT, Label, Entity, Keyword" "CMSEL - Selects a subset of components and assemblies.
...
CMSOPT, CMSMETH, NMODE, FREQB, FREQE, FBDDEF, FBDVAL" "CNCHECK - Provides and/or adjusts the initial status of contact pairs.
...
DDELE, NODE, Lab, NEND, NINC" "DEACT
...
DISPLAY - Specifies \"Display settings\" as the subsequent status topic." "DJ - Specify displacement (or rotation) boundary conditions on the components of relative motion of a joint element.
DJ, ELEM, LABEL, VALUE" "DJDELE - Deletes displacement (or rotation) boundary conditions on the components of relative motion of a joint element.
...
DSET, NODE1, NODE2, NODE3, DDEV" "DSUM - Specifies the double sum mode combination method.
...
EDRUN, Option, Cons" "EDSHELL - Specifies shell computation controls for an explicit dynamics analysis.
...
ENDRELEASE, --, TOLERANCE, Dof1, Dof2, Dof3, Dof4" "ENGEN - Generates elements from an existing pattern.
...
EREFINE, NE1, NE2, NINC, LEVEL, DEPTH, POST, RETAIN" "ERESX - Specifies extrapolation of integration point results.
...
FREQ, FREQ1, FREQ2, FREQ3, FREQ4, FREQ5, FREQ6, FREQ7, FREQ8, FREQ9" "FS - Stores fatigue stress components at a node.
...
HBMAT, Fname, Ext, --, Form, Matrx, Rhs" "HELP - Displays help information on ANSYS commands and element types.
...
HEMIOPT, HRES" "HFANG - Defines or displays spatial angles of a spherical radiation surface for antenna parameter calculations.
...
IMPD, Vpath, Ipath, Vsymm, Isymm" "INRES - Identifies the data to be retrieved from the results file.
...
ISFILE, Option, Fname, Ext, --, LOC, MAT1, MAT2, MAT3, MAT4, MAT5, MAT6, MAT7, MAT8, MAT9, MAT10" "ISTRESS - Defines a set of initial stress values.
...
JPEG, Kywrd, OPT" "JSOL - Stores the relative displacement, relative rotation, reaction forces, and moments for the joint element.
...
M, NODE, Lab1, NEND, NINC, Lab2, Lab3, Lab4, Lab5, Lab6" "MAGOPT - Specifies options for a 3-D magnetostatic field analysis.
...
MFCALC, FNUMB, FREQ" "MFCLEAR - Deletes ANSYS Multi-field solver analysis settings.
...
MFFNAME, FNUMB, Fname" "MFIMPORT - Imports a new field into a current ANSYS Multi-field solver analysis.
...
MFRELAX, Lab, VALUE, Option" "MFRSTART - Specifies a restart time for an ANSYS Multi-field solver analysis.
...
MFTOL, Key, Value" "MFVOLUME - Defines a volume load transfer for an ANSYS Multi-field solver analysis.
...
MGEN, ITIME, INC, NODE1, NODE2, NINC" "MIDTOL - Sets middle step residual criterion values for structural transient analyses.
MIDTOL, KEY , TOLERP, TOLERB" "MITER - Defines a mitered bend in a piping run.
...
MOPT, Lab, Value" "MORPH - Turns morphing on or off.
MORPH, Option" "MOVE - Calculates and moves a node to an intersection.
...
MPDELE, Lab, MAT1, MAT2, INC" "MPDRES - Reassembles existing material data with the temperature table.
...
MSADV, SPNUM, MTHA" "MSAVE - Sets the solver memory saving option. This option only applies to the PCG and DPCG solvers.
...
PCGOPT, Lev_Diff , --, ReduceIO" "PCIRC - Creates a circular area centered about the working plane origin.
...
PIVCHECK, KEY" "PLCAMP - Plots Campbell diagram data for applications involving rotating structure dynamics.
PLCAMP, Option, SLOPE, UNIT, FREQB" "PLCONV - Plots the convergence curve for specified items from a p-method solution.
...
PLF2D, NCONT, OLAY, ANUM, WIN" "PLHFFAR - Displays electromagnetic far fields and far field parameters.
PLHFFAR, Opt, Lab, PHI1, PHI2, NPHI, THETA1, THETA2, NTHETA, RADZ, CS, Comp" "PLLS - Displays element table items as contoured areas along elements.
PLLS, LabI, LabJ, Fact, KUND" "PLNSOL - Displays results as continuous contours.
PLNSOL, Item, Comp, KUND, Fact, FileID" "PLOT - Forms a display.
...
PLSECT, Item, Comp, RHO, KBR" "PLSYZ - Converts and plots scattering, admittance, or impedance parameters as a function of frequency.
PLSYZ, Fname, Ext, Lab, Opt, VAL_I1, VAL_J1, VAL_I2, VAL_J2, VAL_I3, VAL_J3, VAL_I4, VAL_J4" "PLTIME - Defines the time range for which data are to be displayed.
...
PRCAMP, Option, SLOPE, UNIT, FREQB" "PRCONV
...
PRETAB, Lab1, Lab2, Lab3, Lab4, Lab5, Lab6, Lab7, Lab8, Lab9" "PRHFFAR - Prints electromagnetic far fields and far field parameters.
PRHFFAR, Opt, Lab, PHI1, PHI2, NPHI, THETA1, THETA2, NTHETA, RADZ, CS, Comp" "PRI2 - Creates a polygonal area or a prism volume by vertices (GUI).
...
PRISM, Z1, Z2" "PRITER
PRITER - Prints solution summary data." "PRJSOL - Print joint element output.
PRJSOL, Label, Comp" "PRNLD - Prints the summed element nodal loads.
PRNLD, Lab, TOL, Item" "PRNSOL - Prints the nodal solution results.
PRNSOL, Item, Comp" "PROD - Multiplies variables.
PROD, IR, IA, IB, IC, Name, --, --, FACTA, FACTB, FACTC" "PRPATH - Prints path items along a geometry path.
...
PRSYZ, Fname, Ext, Lab, Opt" "PRTIME - Defines the time range for which data are to be listed.
...
RDELE, NSET1, NSET2, NINC" "REAL - Sets the element real constant set attribute pointer.
...
REDUCE, NLOC, LENG, ELEM" "REFLCOEF - Calculates the voltage reflection coefficient (REFLC), standing wave ratio (VSWR), and return loss (RL) in a COAX fed deviceREMESH - Specifies the starting and ending remeshing points for rezoning.
REFLCOEF, Portin, Pvolt, Pang, Pdist, Vpathy" "REORDER
...
RESUME, Fname, Ext, --, NOPAR, KNOPLOT" "REXPORT - Exports displacements from an implicit run to ANSYS LS-DYNA.
...
SDELETE, SFIRST, SLAST, SINC, KNOCLEAN" "SE - Defines a superelement.
SE, File, --, --, TOLER" "SECCONTROLS - Overrides program calculated properties.
SECCONTROLS, VAL1, VAL2, VAL3, VAL4, VAL5, VAL6, VAL7, VAL8, VAL9, VAL10, VAL11" "SECDATA - Describes the geometry of a section.
...
SECFUNCTION, TABLE" "SECJOINT - Defines local coordinate systems at the nodes that form the MPC184 joint element.
SECJOINT, --, ID1, ID2" "SECLOCK - Specifies locks on the components of relative motion in a joint element.
...
SPACE, NODE" "SPARM - Calculates scattering (S) parameters between ports of a network system.
...
SPSWP, FREQB, FREQE, FREQINC, SwpOpt, EFAcc, OutPut, FileOpt" "SPTOPT
...
SSTIF, Key" "SSUM
SSUM - Calculates and prints the sum of element table items." "STAOPT - Specifies static analysis options.
STAOPT, Method" "STAT
...
SUCR, SurfName, SurfType, nRefine, Radius" "SUDEL - Delete geometry information as well as any mapped results for specified surface.
...
SWLIST, Ecomp" "SXCLR - Clears the Variational Technology database.
SXCLR, Type" "SXDISC - Defines an element component as a discrete input variable for the DesignXplorer VT.
SXDISC, Name, ElComp" "SXEVAL - Triggers evaluation of generated results based on input variables specified via the SXVMOD command.
SXEVAL, --, Mode" "SXFREQ - Defines the frequency as input variable for the FS Module or the DesignXplorer VT.
SXFREQ, Name, MIN, MAX, INC, RedOpt" "SXGEOM - Defines a geometry parameter created with ANSYS ParaMesh as a DesignXplorer VT input variable.
SXGEOM, Name, MIN, MAX, RedOpt, Order" "SXIN - Defines an inertial load as an input variable for DesignXplorer VT.
SXIN, Name, MIN, MAX, RedOpt, Lab, Comp, --, VarType, Order" "SXMETH - Defines the solution options for the DesignXplorer VT.
SXMETH, SoluType, ApprType, ModeTrack" "SXMP - Defines a material property as an input variable for DesignXplorer VT.
SXMP, Name, MIN, MAX, RedOpt, Lab, MAT, ElComp, VarType, Order" "SXOP - Defines options value for the DesignXplorer VT.
SXOP, Oper, Name, Value" "SXPOST
SXPOST - Launches the DesignXplorer VT postprocessing application." "SXREAL - Defines a real constant property as an input variable for the DesignXplorer VT.
SXREAL, Name, MIN, MAX, RedOpt, Lab, NSET, ElComp, VarType, Order" "SXRFIL - Specifies the file to which DesignXplorer VT results are written.
SXRFIL, Fname, Ext, Dir" "SXRSLT - Defines a result quantity for the DesignXplorer VT.
SXRSLT, Name, Entity, Type, Comp, ACC, CompName" "SXSEC - Defines a section property as an input variable for the FS Module or the DesignXplorer VT.
SXSEC, Name, MIN, MAX, RedOpt, Lab, SECID, LAYERID, ElComp, VarType, Order" "SXSFE - Defines a surface load as an input variable for the DesignXplorer VT.
SXSFE, Name, MIN, MAX, RedOpt, Lab, LKEY, ElComp, VarType, Order" "SXSL - Selects a subset of elements associated with an SX input variable.
SXSL, Type, Varname" "SXSTAT - Print the status of the DesignXplorer VT definitions and settings into a separate window.
SXSTAT, EntyLis" "SXTEMP - Defines the temperature as input variable for the DesignXplorer VT.
SXTEMP,Name, MIN, MAX, RedOpt, --, VarType, Order" "SXVMOD - Modifies the status or current value of an input variable for the DesignXplorer VT.
SXVMOD, Name, Oper, Value" "TALLOW - Defines the temperature table for safety factor calculations.
...
USRCAL, Rnam1, Rnam2, Rnam3, Rnam4, Rnam5, Rnam6, Rnam7, Rnam8, Rnam9" "V - Defines a volume through keypoints.
...
VPUT, Par, IR, TSTRT, KCPLX" "VROTAT - Generates cylindrical volumes by rotating an area pattern about an axis.
...
~CATIAIN, Name, Extension, Path, - -, - -, BLANK, - -" "~CIFIN - Transfers an electronic layer geometry file in CIF format into the ANSYS program.
~CIFIN, Filename, Ext, Dir, Scale, Offset" "~PARAIN - Transfers a Parasolid file into the ANSYS program.
...
font-lock-type-face      keep)     (3     font-lock-constant-face
keep)               (4               (quote               shadow)
keep))  ("\\(^\\|\\$\\)\\s-*\\(/SX\\)\\>"  (2 font-lock-type-face
keep)) ("\\(^\\|\\$\\)\\s-*\\(SWLI\\)\\(\\(?:S\\(?:T\\)?\\)?\\)\\(\\w*\\)" (2
font-lock-type-face      keep)     (3     font-lock-constant-face
keep)               (4               (quote               shadow)
keep)) ("\\(^\\|\\$\\)\\s-*\\(SWGE\\)\\(\\(?:N\\)?\\)\\(\\w*\\)" (2
...
keep))        ("\\(^\\|\\$\\)\\s-*\\(CALC\\)\\(\\w*\\)"        (2
font-lock-type-face       keep)      (3       (quote      shadow)
keep))   ("\\(^\\|\\$\\)\\s-*\\(C\\)\\>"  (2  font-lock-type-face
keep)) ("\\(^\\|\\$\\)\\s-*\\(BUCO\\)\\(\\(?:P\\(?:T\\)?\\)?\\)\\(\\w*\\)" (2
font-lock-type-face      keep)     (3     font-lock-constant-face
keep)               (4               (quote               shadow)
keep))        ("\\(^\\|\\$\\)\\s-*\\(BTOL\\)\\(\\w*\\)"        (2
...
(defconst ansys-font-lock-keywords	;NEW
  (append
   ...
   '(("^\\s-*/SYS\\s-*,\\(.\\{1,75\\}\\)$" 1 'font-lock-doc-face keep))
...
(defvar ansys-previous-major-mode ""	;NEW
  "The previous major mode, before Ansys mode.")

(defun ansys-toggle-mode nil	   	;NEW FIXME this toggles also all ansys minor-hooks?
  "Restore the previous major mode, if possible."
  (interactive)
...
* Code finding and navigation: Code lines, number blocks,
  ...
  navigiation in lisp mode, when you interchange list and sexp
  with block.
...
* Sophisticated highlighting (optionally also user variables)
  ...
  interpreter logic as closely as possible. (` *' is still a
  comment string although deprecated! See the manual to *LET.)

  The fontification distinguishes between Ansys keywords (or
  commands), parametric-functions and element names.  For the
  Ansys commands the unique stem (at least 4 characters) is
  slightly distinguished from the rest of the complete command
  name.  In case of arbitrary characters after the unique word
...
* Auto-insertion of code templates into new APDL files
  ...
  Include the following lisp expressions in your .emacs file,
...
`ansys-lmutil-program'
  License manager utility program with complete path specification.
  Necessary for the command `ansys-license-status'.

`ansys-license-file'
  License file or license server specification.
  The license server specification must include the port number
  when it isn't 1055, the default port number:
  port_number@server_name.

`ansys-help'
  The Ansys help system executable with complete path specification
...
;  (make-local-variable 'font-lock-defaults) is always local
  ...
  (setq ansys-format (intern "mac"))	;FIXME: redundant?
					;? why intern?
  ;; menu
  (or (string= system-type "darwin") ;FIXME: might work on darwin system as well
...
(defun ansys-show-paren-mode ()
  "Switch on function `show-paren-mode'.
Matching parenthesis are highlighted."
  (show-paren-mode 1))

(defun ansys-ruler-mode ()
  "Switch on `ruler-mode'.
...
(defun ansys-show-command-parameters ()	;NEW FIXME: parameter help for get-functions!"
  "Displays the Ansys command parameters.
Do this for the previous Ansys command in the current line.  See
`ansys-command-start'"
  (interactive );"sCommand string: ")
  (let ((case-fold-search t))
    (save-excursion
      (unless (ansys-in-indentation-p)
	(ansys-command-start))		;this isn't working yet for all functions
      (re-search-forward "[^[:space:]]\\w*\\>" nil t)
      (catch 'foo
	(dolist (s ansys-dynamic-prompt)
	  (when (string-match (concat "^" (match-string 0)) s)
	    (momentary-string-display
	     (propertize (concat s "\n") 'font-lock-face 'tooltip)
	     (window-start))
	    (throw 'foo nil)))
	(error "\"%s\" not found in command list" (match-string 0))))))
...
(defconst ansys-mode-menu
  (list "Ansys"
	...
	["Start Ansys help system" ansys-start-ansys-help]
	["Display License Status" ansys-license-status]
	["Show Command Parameters"      ansys-show-command-parameters]
	["Display User Variables" ansys-display-variables]
	["Show Temporary Ruler"         ansys-column-ruler]
...
futher number line is in the file signal an error."
  ...
  (let ((re "^\\(\\s-*[-+]?[[:digit:]]+\\.?[[:digit:]]*E?[+-]?[[:digit:]]*\\)+\\s-*$"))
...
(defun ansys-back-to-msg-command ()	;NEW
  "Move point back to the beginning of a previous *MSG command.
Signals an error when there is no *MSG command."
  (interactive)
  (when (re-search-backward "\\*[mM][sS][gG]") ;signals error when no *MSG
      (back-to-indentation)))

(defun ansys-move-to-end-of-format-string () ;NEW
  "Move cursor to the end of an *MSG command format string."
  (interactive)
  (when (ansys-msg-line-p)
    (forward-line))
...
(defun ansys-move-before-comment()	;NEW
  (interactive)
  (beginning-of-line)
...
statement or to the line's first command.  When in a *MSG format
string move backward to the beginning of the respective *MSG
command.  When no *MSG command is to be found signal an error.
...
`$' statement or to the line's end.  Otherwise, when in a *MSG
format string move forward to the end of the possibly multi-line
format string.  In a code comment skip back to the end of the
uncommented code, except when at the end of the code text, then
skip to the next code line's end."
  (interactive "p")
  (unless num (setq num 1))
  (while (> num 0)
    (cond
...
stops in the direction of the outer loops (up to higher levels)."
	...
	(catch 'foo			;end the inner while loop
	  (while (or
		...
		(if (< min-level-offset 0)
		    (error "Minimum block level - as specified - reached")
		  (error "Maximum block level - as specified - reached")))
...
(defun ansys-mark-block ()
  "Mark current block level.
\(Which is to say the cursor must be inside the respective block)"
  (interactive)
  (ansys-up-block)
  ...
  (when ansys-blink-matching-block
	...
	(save-excursion
	  ...
	  (setq pos (match-end 0)
		...
		bb-arg (save-excursion	;FIXME: bb-arg is eating commas
			 (save-restriction
			   (goto-char pos)
...
(define-skeleton ansys-skeleton		;NEW
  ...
  "!!  the _return and _status variables (_STATUS: 0 no error, 1" \n
  ...
  "!! /input," \n
  "/filname," (setq ansys-job (skeleton-read "Jobname: " "file")) \n
  \n
  "!! ==============================" \n
...
(define-ansys-skeleton ansys-if
  "Insert an if statement in the current format's syntax." ;FIXME: docu is wrong
  (format "Value/Parameter 1: "
	  "*IF," str ","
	  (completing-read "Operator: [LT] (TAB for completion list) "
			   '("EQ","NE","GT","LE","GE","ABLT","ABGT") ;LT default
			   nil
			   nil
			   "";initial input
			   nil
			   "LT")	;default
	  ","
	  (read-from-minibuffer "Value/Parameter 2 [1]: " nil nil '(1 2 3 4) 1)
	  ","
	  (read-string "Action: (:label,STOP,EXIT,CYCLE) ")
	  \n >_
	  "*ENDIF" > \n)		;FIXME endif not necessary for all actions
  (mac . format))
...
(define-ansys-skeleton ansys-mp		;FIXME: a bit over the top
  "Insert an if statement in the current format's syntax."
...
when set.  The file (JOBNAME.abt) in the default directory
contains the sole word \"nonlinear\".  The function prompts for
an appropriate job name when ARG is negative, otherwise it uses a
sensible default.  In case the default directory is not of your
liking, use: `M-x cd'."
  (interactive "p")
  (unless arg (setq arg 0))
;  (debug)
...
(defun ansys-display-error-file ()	;NEW
  "Open the current Ansys error file in the default directory.
When variable `ansys-job' is not given use \"file.err\"."
  (interactive)
  (let (file)
      (if ansys-job
	  (setq file ansys-job)
...
Alternatively one could use the Ansys \"/SYS, anshelp100\"
command, when anshelp100 (or anshelp100.chm on Windows) is in the
PATH variable."
  (interactive)
...
(defun ansys-replot ()		;NEW PanZoomRotate box
  "Replot in interactive graphics mode."
  (interactive)
  (unless (string= "run" (process-status ansys-process))
    (error "No Ansys process is running"))
...
And specify it in the variable `ansys-job'."
  (interactive)
  (if ansys-job
      (setq ansys-job
  ...
  (message (concat "Set job-name to \"" ansys-job "\".")))
...
And specify it in the variable `ansys-license'."
  ...
  (let ((lic (if (not (string= ansys-license ""))
		ansys-license
	      "struct")))
    (setq ansys-license
	  (completing-read (concat "License type [" lic "]: ")
			   '("ansys" "struct" "ane3" "ane3fl" "ansysds" "preppost")
			   nil nil nil nil lic))
