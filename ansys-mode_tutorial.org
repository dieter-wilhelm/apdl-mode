...
** Explore the ANSYS-Mode menu
  ...
  dependent stuff, like opening the ANSYS help browser from the
  ANSYS-Mode menu (or typing its keyboard shortcut *C-c C-h*).
...
** Select and insert APDL templates into your code \\ Screenshot with the ANSYS Classics GUI (left) and Emacs (right) on Win64 
...
* Summary
** Use 3 levels with ANSYS-Mode appropriate to your needs \\ Since the relevance of APDL remains tall: `WorkBench' operates *exclusively* the ANSYS solver with it!  While for many applications WorkBench is taking over the "code authoring" part from the user, use APDL for *repetitive* tasks and command snippets, *extending* WB.
...
#         :BEAMER_col: 0.5
    ...
    highlighting, checking APDL commands with the `online' help.
*** Earnest APDL *editor*
         :PROPERTIES:
         :BEAMER_env: block
         :BEAMER_envargs: C[t]
