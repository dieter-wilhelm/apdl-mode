...
** APDL Keyword Documentation
   ...
   command.  When using a prefix argument (typing C-u before C-c C-b)
   it inquires the keyword from you.  Keywords beginning with a
   quotation mark ‘"’ describing general manual sections, for example
   typing ‘"SHELLS"’ will call the collection of all shell elements in
   the Ansys manual.  You can complete your input with the <TAB> key.
...
* MAPDL Solver Control
  and communication (mainly restricted to GNU-Linux systems).
