...
** GNU Emacs editor support for working with ANSYS FEA
   ...
   Analysis) suite [[http://www.ansys.com][ANSYS]]. It is *not only* a syntax highlighter for
   APDL (ANSYS Parametric Design Language) code!
#+LaTeX: \\[.2cm]
   It provides *ANSYS-Mode*, an add-on to the [[http://www.gnu.org/software/emacs/][Gnu Emacs]] editor for
   navigating, inspecting and writing APDL files.  The mode offers
   managing and communication capabilities for ANSYS processes, like
   interactive code debugging with the ANSYS solver or listing the
   current license status, etc.
#+LaTeX: \\[.2cm]
...
** Download ANSYS-Mode together with the free Emacs editor \\ There are no license restrictions also for commercial use
  ...
  conjunction with the latest Emacs editor binary from the Google Code
