...
** Take a short-cut to the code template preview
   Type *C-c C-s* to select a template name, use the <TAB> key to open
   a completion window of available subjects.
...
** Check auto-insertion and outline your code (tree view)
  Create a new `macro' with the file suffix `.mac' and let Emacs
  auto-insert a code skeleton with `outline' headings.  Collapse it to
  a tree view with *C-c @ C-t* and open all (*C-c @ C-a*) again.
