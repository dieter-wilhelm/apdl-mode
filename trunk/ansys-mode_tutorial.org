...
# - show a copied result to the apdl window from calc in calculator.png (d f 3)
...
** Select and insert APDL code templates into your code \\ Screenshot the ANSYS Classics GUI (left) and Emacs (right) on Win64 
...
** Outline your code and hide the entries (tree view)
  Create a new APDL file and let Emacs auto-insert an APDL skeleton.
  Collapse it to a tree with *C-c @ C-t* and open (*C-c @ C-a*) all
  again. '!@', '!@@' are the outline heading strings.

#+ATTR_LaTeX: height=7.5cm
  [[./outline.png]]
  
** Utilise the completion of around 2000 APDL symbols
  Move the cursor behind a character (here 'c') or word fragment and
  type *ESC TAB* or *C-M-i* for completing all APDL command-, element-
  and function names.
...
#+ATTR_LaTeX: height=6.5cm
  [[./process.png]]
** Display license state and some auxiliary processes
   In the middle section you see the license state (*C-c C-l*) and
   left below an Emacs thumbnail view of images in a folder.
...
* Summary
** Use three levels of ANSYS-Mode to your needs and liking
*** Simple APDL viewer
         :PROPERTIES:
         :BEAMER_env: block
         :BEAMER_envargs: C[t]
#         :BEAMER_col: 0.5
         :END:
    Navigation, viewing (highlighted code), inspecting APDL with the
    inbuilt 'online' help.
*** Advanced APDL editor
         :PROPERTIES:
         :BEAMER_env: block
         :BEAMER_envargs: C[t]
#         :BEAMER_col: 0.5
    ...
    auto-indentation, abbreviations, auto-insertion, all Emacs'
    convenience features, ...
*** Hardcore APDL processing environment
         :PROPERTIES:
         :BEAMER_env: block
         :BEAMER_envargs: C[t]
#         :BEAMER_col: 0.5
         :END:
    Hybrid between coding and debugging with solver communication
    (UNIX only), retrieving license stati, error file viewing, abort
    file handling, extending ANSYS-Mode, ...
